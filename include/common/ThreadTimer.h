  ...
  class TimerNode {
   public:
    TimerNode(int id, int delay, int interval, std::function<void(void*)> cb, void* params = nullptr)
      ...
      if (delay == 0) {
        m_expire =
            std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() + m_interval);
      } else {
        m_expire = std::chrono::time_point_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now() + std::chrono::microseconds(delay * MICROSECONDS_PER_MILLISECOND));
      }
    }
