...
/**
 * @brief This is the size of the arrays ModeControlsA and ModeControlsB contained in the structure @link ControlMapping
 * @endlink .
 */
...
enum POSITION_TYPE {
  ...
  CARTESIAN_POSITION =
      1, /*!< A cartesian position described by a translation X, Y, Z and an orientation ThetaX, thetaY and ThetaZ. */
  ANGULAR_POSITION = 2, /*!< An angular position described by a value for each actuator. */
  RETRACTED = 3,        /*!< The robotic arm is in retracted mode. It may be anywhere between the HOME position and the
                           RETRACTED position. */
...
enum TORQUECONTROL_TYPE { DIRECTTORQUE = 0, IMPEDANCEANGULAR = 1, IMPEDANCECARTESIAN = 2 };
...
enum CONTROL_MODULE {
  ...
  /**
   * @brief Cartesian velocity control mode. values sent to the end effector are velocity. translation Unit: meter /
   * second, orientation unit: RAD / second.
   */
...
enum RETRACT_TYPE {
  ...
  /**
   * @brief The robotical arm is initialized and is anywhere in the workspace but it is not retracted, in READY position
   * or between.
   */
...
/** @brief This data structure holds values in an angular(joint by joint) control context. As an example struct could
 * contains position, temperature, torque, ... \struct AngularInfo KinovaTypes.h "Definition"
 */
struct AngularInfo {
  /**
   * As an example if the current control mode is angular position the unit will be degree but if the control mode is
   * angular velocity then the unit will be degree per second.
...
struct CartesianInfo {
  /**
   * As an example if the current control mode is cartesian position the unit will be meters but if the control mode is
   * cartesian velocity then the unit will be meters per second.
  ...
  /**
   * This is the value related to the orientation around the X axis. Depending on the context it's been used, the unit
   * of the value will change. As an example if the current control mode is cartesian position the unit will be RAD but
   * if the control mode is cartesian velocity then the unit will be RAD per second.
   * @brief This is the value related to the orientation around the X axis. Unit depends on the context it's been used.
   */
  float ThetaX;

  /**
   * As an example if the current control mode is cartesian position the unit will be RAD but if the control mode is
   * cartesian velocity then the unit will be RAD per second.
...
/**
 * If the robot is a Jaco, the 3 finger values will be filled but if the robot is a Mico, the finger 3 will not be
 * filled.
...
struct Limitation {
  /**
   * @brief In a cartesian context, this represents the translation velocity but in an angular context, this represents
   * the velocity of the actuators 1, 2 and 3.
   */
  float speedParameter1;

  /**
   * @brief In a cartesian context, this represents the orientation velocity but in an angular context, this represents
   * the velocity of the actuators 4, 5 and 6.
   */
...
/**
 *  @brief This data structure represents an abstract position built by a user. Depending on the control type the
 * Cartesian information, the angular information or both will be used. \struct UserPosition KinovaTypes.h "Definition"
 */
struct UserPosition {
  /**
...
/** @brief This data structure represents a point of a trajectory. It contains the position a limitation that you can
 * applied. \struct TrajectoryPoint KinovaTypes.h "Definition"
 */
struct TrajectoryPoint {
  ...
  /**
   * @brief A flag that indicates if the tracjetory's synchronization is active. (1 is active 0 is not). ONLY AVAILABLE
   * IN ANGULAR CONTROL.
   */
  int SynchroType;
...
struct JoystickCommand {
  ...
  /**
   * @brief That holds the behaviour of the stick when it is inclined from left to right. (value between -1 and 1
   * inclusively) 2 functionalities can be mapped with this value, there is an event when the value is negative and
   * there is one when it is positive.
   */
  float InclineLeftRight;

  /**
   * @brief That holds the behaviour of the stick when it is inclined forward and backward. (value between -1 and 1
   * inclusively) 2 functionalities can be mapped with this value, there is an event when the value is negative and
   * there is one when it is positive.
   */
  ...
  /**
   * @brief That holds the behaviour of the stick when it is rotated clockwork and counter clockwork. (value between -1
   * and 1 inclusively) 2 functionalities can be mapped with this value, there is an event when the value is negative
   * and there is one when it is positive.
   */
  float Rotate;

  /**
   * @brief That holds the behaviour of the stick when it is moved from left to right. (value between -1 and 1
   * inclusively) 2 functionalities can be mapped with this value, there is an event when the value is negative and
   * there is one when it is positive.
   */
  float MoveLeftRight;
  ...
  /**
   * @brief That holds the behaviour of the stick when it is moved forward and backward. (value between -1 and 1
   * inclusively) 2 functionalities can be mapped with this value, there is an event when the value is negative and
   * there is one when it is positive.
   */
   ...
   * 2 functionalities can be mapped with this value, there is an event when the value is negative and there is one when
   * it is positive.
   */
  float PushPull;
 ...
 * it will be done here. The easiest way to modify the client configuration is to get the current one by calling the
 * function GetClientConfigurations, modify all the parameters you need and send the structure back to the robot by
 * calling the function SetClientConfigurations(). Note that some of the parameters are read only. That means that even
 * if you modify them and send the structure to the robot, they will not be modified.
 */
struct ClientConfigurations {
  ...
  /**
   * @brief This is where you can store the serial number of the robot. The string must ends with a EndOfString
   * character(\0).
   */
  char Serial[STRING_LENGTH];

  /**
   * @brief This is where you can store the model number of the robot. The string must ends with a EndOfString
   * character(\0).
   */
  char Model[STRING_LENGTH];

  /**
   * @brief That tells you if the robot is left handed or right handed. If you modify this parameter, the change will
  ...
  /**
   * @brief This is a flag to invert finger 2 and 3. It is mostly used in rehab for left handed robot.(0 = normal, 1 =
   * inverted)
   */
  ...
  /**
   * @brief It is a flag that indicates if the GOTO(pre programmed) position are deleted when the robot goes in READY
   * position.
   */
...
enum ControlFunctionalityTypeEnum {
  ...
  /**
   * @brief Home the robot if the is initialized and anywhere in the workspace except between the READY and RETRACTED
   * position. Go to RETRACTED position if the robot is in READY position and go to READY position if the robot is in
   * RETRACTED position.
   * is in READY mode.
   */
...
/**
 * @brief This is an event from a controller's stick. Each variable of the struct can be mapped with a
 * ControlFunctionalityTypeEnum.
 */
struct StickEvents {
  /**
...
/**
 * @brief This is an event from a controller's button. Each variable of the struct can be mapped with a
 * ControlFunctionalityTypeEnum.
 */
struct ButtonEvents {
  /**
 ...
 *
 * As an example, the kinova 3-axis joystick has its own control mapping. This API also has its own control mapping.
 * Note that since list A and list B cannot be used at the same time in the same control mapping, it implies that either
 * one of the variable can have a >= 0 value.
 */
struct ControlMapping {
  ...
  /**
   * @brief This is the mode map list A. By default, on the 3-axis kinova joystick, it corresponds to the modes
   * accessible with the left button on the top of the stick.
   */
  ControlsModeMap ModeControlsA[MODE_MAP_COUNT];

  /**
   * @brief This is the mode map list B. By default, on the 3-axis kinova joystick, it corresponds to the modes
   * accessible with the right button on the top of the stick.
   */
  ControlsModeMap ModeControlsB[MODE_MAP_COUNT];
};

/**
 * @brief This structure holds all the control mapping of the system. It is the entry point if you want to use the
 * mapping system.
 */
...
struct ZoneLimitation {
  /**
   * @brief The first speed parameter. Used in angular control, it is the velocity of the actuator 1, 2 and 3 and if
   * used in cartesian control, it is the translation velocity.
   */
  float speedParameter1;

  /**
   * @brief The second speed parameter. Used in angular control, it is the velocity of the actuator 4, 5 and 6 and if
   * used in cartesian control, it is the orientation velocity.
   */
  float speedParameter2;
...
struct QuickStatus {
  ...
  /**
   * @brief This flag indicates the active control module. This value is directly associated with the enum
   * CONTROL_MODULE.
   */
...
struct Zone {
  ...
  /**
   * @brief Limitation to apply inside the protection zone. As an example, if you want the zone to be am unaccessible
   * zone, you need to limit the velocity to 0.
   */
