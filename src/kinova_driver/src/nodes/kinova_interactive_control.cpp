...
// %Tag(1DOF)%
void make1DofMarker(const std::string& frame_id, const std::string& axis,
                    unsigned int interaction_mode, const tf::Vector3& position,
...
void sendFingerGoal(
    const visualization_msgs::InteractiveMarkerFeedbackConstPtr& feedback) {
  Finger_actionlibClient client(
      "/" + tf_prefix_ + "_driver/fingers/finger_positions", true);
  ...
  markerPos =
      std::min(maxMarkerPose, std::max(0.0f, float(feedback->pose.position.x)));
...
void sendArmPoseGoal(geometry_msgs::PoseStamped& endeffector_pose) {
  ArmPose_actionlibClient client(
      "/" + tf_prefix_ + "_driver/pose_action/tool_pose", true);
...
void sendArmJointGoal(const std::string marker_name, double joint_offset) {
  ArmJoint_actionlibClient client(
      "/" + tf_prefix_ + "_driver/joints_action/joint_angles", true);
  ...
  switch (feedback->event_type) {
    ...
    case visualization_msgs::InteractiveMarkerFeedback::MOUSE_DOWN:
      ...
      if (mouse_was_up = true) {
        ...
        tf::Matrix3x3(quaternion_mousedown)
            .getRPY(roll_mousedown, pitch_mousedown, yaw_mousedown);
        if (feedback->marker_name == "cartesian_6dof") {
          ROS_INFO_STREAM("cartesian_6dof control mode.");
        } else {
          ROS_INFO_STREAM("Joint control is mode.");
          //                ROS_INFO_STREAM( s.str() << ": mouse DOWN (refers to
          //                current status): "
          ...
          //                                 << " time: " <<
          //                                 feedback->header.stamp.sec << "sec,
          //                                 "
          //                                 << feedback->header.stamp.nsec << "
          //                                 nsec" );
        }
      }
    ...
    case visualization_msgs::InteractiveMarkerFeedback::MOUSE_UP:
      ...
      tf::Matrix3x3(quaternion_mouseup)
          .getRPY(roll_mouseup, pitch_mouseup, yaw_mouseup);
      if (feedback->marker_name == "cartesian_6dof") {
        ...
        //            ROS_INFO_STREAM( s.str() << ": mouse UP (refers to
        //            command): "
        //                             << "\nposition = "
        ...
        //            ROS_INFO_STREAM("Pose_endeffector parent frame is : " <<
        //            pose_endeffector.header.frame_id);
        ...
        //                             << feedback->header.stamp.nsec << "
        //                             nsec");
        sendArmJointGoal(feedback->marker_name,
                         (yaw_mouseup - yaw_mousedown) * 180 / M_PI);
      }
...
// %Tag(CurrentJoint)%
void currentJointsFeedback(
    const kinova_msgs::JointAnglesConstPtr joint_command) {
...
void currentPoseFeedback(const kinova_msgs::KinovaPoseConstPtr pose_command) {
  ...
  if (getCurrentCommand == false) {
    ...
    pose.setOrigin(tf::Vector3(current_pose_command.X, current_pose_command.Y,
                               current_pose_command.Z));
    tf::Quaternion q = kinova::EulerXYZ2Quaternion(current_pose_command.ThetaX,
                                                   current_pose_command.ThetaY,
                                                   current_pose_command.ThetaZ);
    pose.setRotation(q);
    make6DofMarker(false, visualization_msgs::InteractiveMarkerControl::NONE,
                   pose, true);
...
int main(int argc, char** argv) {
  ...
  } else  // there is an input...
  {
    ...
    //        {
    //            ROS_WARN("Invalid kinova_robotType error! Obtained: %s.",
    //            kinova_robotType_.c_str()); return -1;
    //        }
  ...
  //    boost::lexical_cast<string>(same_type_index); // in case of multiple
  //    same_type robots
  ...
  ros::Subscriber armJoint_sub =
      nh.subscribe("/" + tf_prefix_ + "_driver/out/joint_command", 1,
                   &currentJointsFeedback);
  ros::Subscriber armCartesian_sub =
      nh.subscribe("/" + tf_prefix_ + "_driver/out/cartesian_command", 1,
                   &currentPoseFeedback);

  armJoint_interMark_server.reset(
      new interactive_markers::InteractiveMarkerServer(
          tf_prefix_ + "_interactive_control_Joint", "", false));
  armPose_interMark_server.reset(
      new interactive_markers::InteractiveMarkerServer(
          tf_prefix_ + "_interactive_control_Cart", "", false));

  ros::Duration(0.1).sleep();

  // %Tag(CreatInteractiveMarkers)%
  // Cartesian Control Marker build based on current pose in
  // currentPoseFeedback().

  // Joint Control
  tf::Vector3 position = tf::Vector3(0, 0, 0);
  make1DofMarker(tf_prefix_ + "_link_1", "z",
                 visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                 position, "1st Axis", "marker_joint1");
  make1DofMarker(tf_prefix_ + "_link_2", "z",
                 visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                 position, "2nd Axis", "marker_joint2");
  make1DofMarker(tf_prefix_ + "_link_3", "z",
                 visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                 position, "3rd Axis", "marker_joint3");
  make1DofMarker(tf_prefix_ + "_link_4", "z",
                 visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                 position, "4th Axis", "marker_joint4");
  if (arm_joint_number_ == 6) {
    make1DofMarker(tf_prefix_ + "_link_5", "z",
                   visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                   position, "5th Axis", "marker_joint5");
    make1DofMarker(tf_prefix_ + "_link_6", "z",
                   visualization_msgs::InteractiveMarkerControl::ROTATE_AXIS,
                   position, "6th Axis", "marker_joint6");
  }
