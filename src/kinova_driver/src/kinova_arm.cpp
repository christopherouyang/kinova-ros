...
KinovaArm::KinovaArm(KinovaComm &arm, const ros::NodeHandle &nodeHandle, const std::string &kinova_robotType,
  ...
  //    tf_prefix_ = kinova_robotType_ + "_" + boost::lexical_cast<string>(same_type_index); // in case of multiple
  //    same_type robots
  ...
  for (int i = 0; i < finger_number_; i++) {
    joint_names_[arm_joint_number_ + finger_number_ + i] =
        tf_prefix_ + "joint_finger_tip_" + boost::lexical_cast<std::string>(i + 1);
  }
  ...
  clear_trajectories_ =
      node_handle_.advertiseService("in/clear_trajectories", &KinovaArm::clearTrajectoriesServiceCallback, this);

  set_force_control_params_service_ =
      node_handle_.advertiseService("in/set_force_control_params", &KinovaArm::setForceControlParamsCallback, this);
  start_force_control_service_ =
      node_handle_.advertiseService("in/start_force_control", &KinovaArm::startForceControlCallback, this);
  stop_force_control_service_ =
      node_handle_.advertiseService("in/stop_force_control", &KinovaArm::stopForceControlCallback, this);
  set_actuator_torques_to_zero_ =
      node_handle_.advertiseService("in/set_zero_torques", &KinovaArm::setJointTorquesToZeroService, this);
  run_COM_parameter_estimation_service_ = node_handle_.advertiseService(
      "in/run_COM_parameters_estimation", &KinovaArm::runCOMParameterEstimationService, this);

  set_end_effector_offset_service_ =
      node_handle_.advertiseService("in/set_end_effector_offset", &KinovaArm::setEndEffectorOffsetCallback, this);

  start_null_space_service_ =
      node_handle_.advertiseService("in/set_null_space_mode_state", &KinovaArm::ActivateNullSpaceModeCallback, this);
  set_torque_control_mode_service_ =
  ...
  /* Set up Subscribers*/
  joint_velocity_subscriber_ = node_handle_.subscribe("in/joint_velocity", 1, &KinovaArm::jointVelocityCallback, this);
  cartesian_velocity_subscriber_ =
      node_handle_.subscribe("in/cartesian_velocity", 1, &KinovaArm::cartesianVelocityCallback, this);
  cartesian_velocity_with_fingers_subscriber_ = node_handle_.subscribe(
      "in/cartesian_velocity_with_fingers", 1, &KinovaArm::cartesianVelocityWithFingersCallback, this);
  joint_torque_subscriber_ =
...
bool KinovaArm::ActivateNullSpaceModeCallback(kinova_msgs::SetNullSpaceModeState::Request &req,
                                              kinova_msgs::SetNullSpaceModeState::Response &res) {
  kinova_comm_.SetRedundantJointNullSpaceMotion(req.state);
}

bool KinovaArm::setTorqueControlModeService(kinova_msgs::SetTorqueControlMode::Request &req,
                                            kinova_msgs::SetTorqueControlMode::Response &res) {
  kinova_comm_.SetTorqueControlState(req.state);
}

bool KinovaArm::setTorqueControlParametersService(kinova_msgs::SetTorqueControlParameters::Request &req,
  ...
  if (node_handle_.getParam("torque_parameters/com_parameters", com_parameters)) {
    ...
    node_handle_.param("torque_parameters/use_estimated_COM_parameters", use_estimated_COM, true);
...
bool KinovaArm::clearTrajectoriesServiceCallback(kinova_msgs::ClearTrajectories::Request &req,
                                                 kinova_msgs::ClearTrajectories::Response &res) {
  kinova_comm_.eraseAllTrajectories();
}

bool KinovaArm::setForceControlParamsCallback(kinova_msgs::SetForceControlParams::Request &req,
...
bool KinovaArm::runCOMParameterEstimationService(kinova_msgs::RunCOMParametersEstimation::Request &req,
                                                 kinova_msgs::RunCOMParametersEstimation::Response &res) {
  kinova_comm_.runCOMParameterEstimation(robot_type_);
}

bool KinovaArm::setEndEffectorOffsetCallback(kinova_msgs::SetEndEffectorOffset::Request &req,
...
void KinovaArm::cartesianVelocityWithFingersCallback(
    const kinova_msgs::PoseVelocityWithFingersConstPtr &cartesian_vel_with_fingers) {
  if (!kinova_comm_.isStopped()) {
    ...
    // We need to see if the fingers have reached the correct position, and if not, set the Fingers command accordingly
