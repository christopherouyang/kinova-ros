...
namespace mm_display
{
    ...
    void mm_display::display_irm(pcl::PointCloud< pcl::PointNormal >::Ptr irm_cloud, double max_range, float new_resolution, pcl::PointCloud<pcl::PointXYZRGB>::Ptr rm_cloud_display)
    {
        display_rm(irm_cloud, max_range, new_resolution, rm_cloud_display);
    }

    void mm_display::display_rm(pcl::PointCloud< pcl::PointNormal >::Ptr rm_cloud, double translation_max_range, float new_resolution, pcl::PointCloud<pcl::PointXYZRGB>::Ptr rm_cloud_display)
    {
        bool patch = false;
        if(patch)   new_resolution = 0.02;
        ...
        for (octomap::OcTree::leaf_iterator it = translation_tree->begin_leafs(max_depth), end = translation_tree->end_leafs(); it != end; ++it)
        {   
            pcl::PointXYZ point;
        ...
        for(int i = 0; i < translation_cloud_filtered->size(); i ++)
        {
            ...
            if(voxel_number > 0)  
            {
                ...
                int count = 0;
                for(auto it = multi_voxel_color.lower_bound(center_point); it != multi_voxel_color.upper_bound(center_point); ++it)
                {
    ...
    void mm_display::display_arrow(pcl::PointCloud< pcl::PointNormal >::Ptr orm_cloud, visualization_msgs::MarkerArray& marker_array, int number)
    {
        display_arrow(*orm_cloud, marker_array, number);
    }
    
    void mm_display::display_arrow(pcl::PointCloud< pcl::PointNormal > orm_cloud, visualization_msgs::MarkerArray& marker_array, int number)
    {
        marker_array.markers.clear();
        for(int i = 0; i < orm_cloud.width; i ++)
        {
            visualization_msgs::Marker marker;
            marker.header.frame_id = "odom";    //这个和rm不一样，rm的基坐标系是agv_base_link，但是小车的位姿是的基础坐标系是odom
            marker.id = i;
