...
bool mm_base_placement::load_rm(int point_number1) {
  ...
  // load rm_cloud
  std::string pcd_file = "mm_n" + std::to_string(point_number1) + "_rm.pcd";
...
void mm_base_placement::get_orm_patch(std::vector<double> eef_pose_vector, std::vector<double> filter_res,
  ...
  for (size_t i = 0; i < indices.size(); i++) {
    ...
    if (distance_to_search_point < 2 * trans_res)
...
bool mm_base_placement::isPoseNearCurrentAgvPose(pcl::PointNormal point, std::vector<double> current_agv_pose,
  ...
  return status_x && status_y && status_theta;
}
...
std::vector<double> mm_base_placement::chooseProperAGVPoseVector(Eigen::Affine3d agv_pose_matrix) {
  ...
  agv_pose_vector[5] = fabs(rpy_angle[0]) < fabs(rpy_angle[3]) && fabs(rpy_angle[1]) < fabs(rpy_angle[4])
...
void mm_base_placement::roll_filter(double angle, pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
  ...
  pcl_filter(angle - ori_res, angle + ori_res, "normal_x", source_cloud, target_cloud);
}

void mm_base_placement::pitch_filter(double angle, pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
                                     pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud) {
  pcl_filter(angle - ori_res, angle + ori_res, "normal_y", source_cloud, target_cloud);
}
...
void mm_base_placement::roll_filter2(double angle, pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
                                     pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud) {
  pcl_filter2(angle, "normal_x", source_cloud, target_cloud);
}

void mm_base_placement::pitch_filter2(double angle, pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
                                      pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud) {
  pcl_filter2(angle, "normal_y", source_cloud, target_cloud);
}

void mm_base_placement::pcl_filter(double left_limit, double right_limit, const std::string& filed_name,
                                   pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
                                   pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud) {
  pass.setInputCloud(source_cloud);
  pass.setFilterFieldName(filed_name);
  pass.setFilterLimits(left_limit, right_limit);
  pass.filter(*target_cloud);
}

void mm_base_placement::pcl_filter2(double angle, const std::string& filed_name,
                                    pcl::PointCloud<pcl::PointNormal>::Ptr source_cloud,
                                    pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud) {
  if (fabs(angle) < PI - ori_res) {
    pcl_filter(angle - ori_res, angle + ori_res, filed_name, source_cloud, target_cloud);
    return;
  }
  ...
  if (angle < -(PI - ori_res)) {  // 靠近-PI
    pcl_filter(-PI, angle + ori_res, filed_name, source_cloud, rm_cloud_filtered_temp1);
    pcl_filter(angle - ori_res + 2 * PI, PI, filed_name, source_cloud, rm_cloud_filtered_temp2);
    // 点云和索引都没有排序，可以对应上的
  } else {  // 靠近PI
    pcl_filter(angle - ori_res, PI, filed_name, source_cloud, rm_cloud_filtered_temp1);
    pcl_filter(-PI, angle + ori_res - 2 * PI, filed_name, source_cloud, rm_cloud_filtered_temp2);
  }
