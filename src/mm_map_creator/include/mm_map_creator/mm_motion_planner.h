...
class mm_motion_planner {
 public:
  ...
  /******************************插值函数******************************/
 public:
  bool cartesionInter(geometry_msgs::Pose start_pose, geometry_msgs::Pose target_pose,
                      std::vector<std::vector<double> >& pose_vector);

 private:
  void linearInter(geometry_msgs::Pose start_pose, geometry_msgs::Pose target_pose,
                   std::vector<geometry_msgs::Pose>& way_point_pose);
  void orientationInter(geometry_msgs::Pose start_pose, geometry_msgs::Pose target_pose,
                        std::vector<geometry_msgs::Pose>& way_point_pose);
  void positionInter(geometry_msgs::Pose start_pose, geometry_msgs::Pose target_pose,
                     std::vector<geometry_msgs::Pose>& way_point_pose);
};
}  // namespace mm_motion_planner
