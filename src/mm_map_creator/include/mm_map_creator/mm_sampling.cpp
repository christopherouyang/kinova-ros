...
namespace mm_sampling
{
    mm_sampling::mm_sampling()
    {
        ...
        load_joint_names(ur_joint_names);
        load_joint_limits(ur_joint_limits);
        srand((int)time(0));   
    }

    bool mm_sampling::rm_down_sample(pcl::PointCloud< pcl::PointNormal >::Ptr rm_cloud, pcl::PointCloud< pcl::PointNormal >::Ptr rm_cloud_down_sample, int sample_number)
    {
...
/****************************************cartesian space sampling****************************************/
    bool mm_sampling::makeCMbyCartesianSpaceSampling(pcl::PointCloud< pcl::PointNormal >::Ptr rm_cloud, pcl::PointCloud< pcl::PointNormal >::Ptr des_rm_cloud, 
                                                     double cm_translation_resolution, double cm_orientation_resolution)
    {
        ...
        std::vector<double> ur_joint_values(6,0);
        std::vector<double> amm_joint_values(9,0);
        std::vector<double> ur_ref_joints = {0, -0.6, 0.9, -0.3, 1.5708, 0};

        for(size_t i = 0; i < ur_ref_joints.size(); i ++)   amm_joint_values[i+3] = ur_ref_joints[i];
        ...
        for(double x = min_x; x <= max_x; x += cm_translation_resolution)
        {
            ...
            {
                if(fabs(atan2(y, x)) > agv_base_ray_range) continue;
                for(double z = initial_height - height_range; z <= initial_height + height_range; z += cm_translation_resolution)
                {
                    des_point.x = x;
                    des_point.y = y; 
                    des_point.z = z;
                    ...
                    for(double roll = initial_roll - roll_range; roll <= initial_roll + roll_range; roll += cm_orientation_resolution)
                    {
                       for(double pitch = initial_pitch - pitch_range; pitch <= initial_pitch + pitch_range; pitch += cm_orientation_resolution)
                       {
                           for(double yaw = initial_yaw - yaw_range; yaw <= initial_yaw + yaw_range; yaw += cm_orientation_resolution)
                           {
                               std::vector<double> target_pose = {x,y,z,roll,pitch,yaw};
                               if(isProperIKExist(target_pose, ur_ref_joints, agv_joint_values, ur_joint_values) && !collision_check.isCollision(ur_joint_values))
                               {
                                    for(int i = 0; i < 6; i ++)  amm_joint_values[i+3] = ur_joint_values[i];
    ...
    bool mm_sampling::isProperIKExist(std::vector<double> target_pose_vector, std::vector<double> ur_ref_joints, std::vector<double> agv_joint_values, std::vector<double>& ur_joint_values)
    {
        ur_joint_values.clear();
        ...
        target_pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(target_pose_vector[3], target_pose_vector[4], target_pose_vector[5]);
        if(mik.getIkFromPose(target_pose, agv_joint_values, ur_joint_values) == false)  return false;

        std::vector<double> max_error(ur_joint_values.size() / 6);
        for(int i = 0; i < max_error.size(); i ++)
        {
            std::vector<double> joint_error(6);
            for(int j = 0; j < 6; j ++)
            {
                joint_error[j] = fabs(ur_joint_values[6 * i + j] - ur_ref_joints[j]);
            }
        ...
        if(min_error > 1.0)  return false;   // 这个值可能需要修改
        else
        {
            //  第k组解是最好的
            int k = std::distance(std::begin(max_error), std::min_element(std::begin(max_error), std::end(max_error)));
            if(ur_joint_values[6*k + 2] < 0.7 || ur_joint_values[6*k + 2] > 2.5) return false;    // 0.7和2.5可能需要修改
            for(int i = 0; i < 6; i ++)
            {
                ur_joint_values[i] = ur_joint_values[6*k + i];
            }
            ur_joint_values.resize(6);  // 只保留前6个数值
            return true;
        } 
    }
    ...
    bool mm_sampling::isPoseinROI(Eigen::Affine3d end_pose_matrix, std::vector<double>& end_pose_vector)
    {
        ...
        double agv_ref_height = 0.393 + ur5_support_height;
        double height_range = 0.5;
        
        if(fabs(atan2(end_pose_matrix(1,3), end_pose_matrix(0,3))) > agv_base_ray_range) return false;
        if(end_pose_matrix(2,3) > agv_ref_height + height_range || end_pose_matrix(2,3) < agv_ref_height - height_range)    return false;
...
/****************************************joint space sampling****************************************/
    ...
    bool mm_sampling::makeCMbyJointSapceSampling(pcl::PointCloud< pcl::PointNormal >::Ptr rm_cloud, int max_sample_point)
    {
        ...
        std::vector<double> manip_vector(max_sample_point);     // 这个vector用来存储机器人在不同位型下的可操作度，方便后续进行归一化
        ...
        {
            joint_space_sampling(ur_joint_values, amm_joint_values);                    // 更新ur和amm关节值
            if(collision_check.isCollision(ur_joint_values))    continue;               // 舍弃碰撞的点
            end_pose_matrix = kinematics.GetTotalHomoMatrix(amm_joint_values);          // eef相对于agv_base_link的齐次变换矩阵

            // double agv_base_ray_range = 30 * PI / 180;
            // double agv_ref_height = 0.393 + ur5_support_height;
            // double height_range = 0.5;
            // if(fabs(atan2(end_pose_matrix(1,3), end_pose_matrix(0,3))) > agv_base_ray_range) continue;
            // if(end_pose_matrix(2,3) > agv_ref_height + height_range || end_pose_matrix(2,3) < agv_ref_height - height_range)    continue;

            convertPoseToPoseVector(end_pose_matrix, end_pose_vector);

            manip_vector[i] = kinematics.GetManipulability(amm_joint_values);
            int percent = double(i) / max_sample_point * 100;
            if(percent > percent_before)
            {
                percent_before += 1;
                cout << "\033[32m" <<  "\r" << "[" << percent << "\%" << "]" << "\033[0m" ;
                for(int j = 0; j <= percent; j ++)  cout << "\033[32m" <<">" <<"\033[0m" ;
            }
            pcl::PointNormal pointnormal;
        ...
        // max manipulability is about 2.23345 for AMM.
        // max_manip = 1.0;

        // for(unsigned int i = 0 ; i < rm_cloud->width; i ++)
        // {
        //     rm_cloud->points[i].curvature = 100 * rm_cloud->points[i].curvature / max_manip;
        // }
    ...
    void mm_sampling::convertPoseToPoseVector(const Eigen::Affine3d pose, std::vector<double>& end_pose_vector)
    {
        ...
        rotation_matrix << pose(0,0), pose(0,1), pose(0,2), pose(1,0), pose(1,1), pose(1,2), pose(2,0), pose(2,1), pose(2,2);
    ...
    int mm_sampling::load_joint_names(std::vector<std::string>& ur_joint_names)
    {
        ur_joint_names.resize(UR_JOINT_NUMBER);
        ros::param::get("ur5_joint_names",ur_joint_names);
        if(ur_joint_names[0] == "")
        {
    ...
    int mm_sampling::load_joint_limits(std::vector<std::pair<double,double> >& ur_joint_limits)
    {
        ur_joint_limits.resize(UR_JOINT_NUMBER);
        for(int i = 0; i < UR_JOINT_NUMBER; i ++)
        {
            ros::param::get("ur5_joint_limits/" + ur_joint_names[i] + "/lower_limits", ur_joint_limits[i].first);
            ros::param::get("ur5_joint_limits/" + ur_joint_names[i] + "/upper_limits", ur_joint_limits[i].second);
        }
        if(!ur_joint_limits[0].first)
        {
            ROS_ERROR("There is no joint names in parameter server!!!");
            exit(0);
        }
    ...
    void mm_sampling::joint_space_sampling(std::vector<double>& ur_joint_values, std::vector<double>& amm_joint_values)
    {
        double times = 100000;
        amm_joint_values[0] = 0;
        amm_joint_values[1] = 0;
        amm_joint_values[2] = 0;
        for(int i = 0; i < UR_JOINT_NUMBER; i ++)
        {
            ur_joint_values[i] = mm_rand(ur_joint_limits[i].first, ur_joint_limits[i].second, times);
            amm_joint_values[i+3] = ur_joint_values[i];
        }
    }
